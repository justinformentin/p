...
export const Sunset = ({ children, title, colorTheme }) => {
  let mouse = { x: 0, y: 0 };
  let cW = 0;
  let cH = 0;
  ...
  function updateSun(e) {
    ...
    const setGrad = (type, grad) => {
      document.getElementById(type).style.background =
        '-webkit-radial-gradient(' + mouse.x + 'px ' + mouse.y + grad;
      document.getElementById(type).style.background =
        '-moz-radial-gradient(' + mouse.x + 'px ' + mouse.y + grad;
      document.getElementById(type).style.background =
        '-ms-radial-gradient(' + mouse.x + 'px ' + mouse.y + grad;
    };
    setGrad('sun', sunGrad);
    ...
    ).style.perspectiveOrigin = ((mouse.x / cW) * 100).toString() + '% -15%';
    document.getElementById('waterReflectionMiddle').style.left =
      (mouse.x - cW - cW * 0.03).toString() + 'px';
  ...
  function dataToggle(kind) {
    ...
    function updatePoints() {
      ...
      const xyDone = currX === endX && currY === endY;
      const yClose =
        kind === 'nightToDay' ? currY >= endY + 2 : currY <= endY + 2;
  ...
  const updateSunset = (colorTheme) =>  colorTheme === 'light' ? nightToDay() : dayToNight();
  useEffect(() => {
    updateDimensions();
    updateSunset(colorTheme)
  }, []);
